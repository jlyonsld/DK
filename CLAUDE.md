...
**Skeleton-migration follow-up (T15.5, not yet built).** Today's `META_PAGE_ACCESS_TOKEN` env var on the DK Supabase project is per-spoke-per-franchise. The right long-term pattern is the **PAR Spoke Skeleton** model described in §4.32: PAR owns one Meta App at the skeleton level, hosts a dispatcher Edge Function that verifies Meta's HMAC and forwards events to the right spoke's webhook signed with `SPOKE_INSTALL_SIGNING_SECRET`. T15.5 lands when T16a lands (they share the OAuth + dispatcher work) — `dk-meta-lead-webhook` switches from "verify Meta's HMAC against my own `META_APP_SECRET` env var" to "verify the PAR forward signature against `SPOKE_INSTALL_SIGNING_SECRET`," and the page token comes from a PAR-side lookup or a cached `dk_config.meta_page_*` mirror instead of `Deno.env.get()`. Charleston re-OAuths once (~30 seconds) and the env vars retire. The leads inbox + reply UI + promote RPC are unchanged — only the receive-and-verify boundary moves.
...
**Critical architecture decision: ONE Meta App at the PAR Spoke Skeleton level — App-Reviewed once, serves every spoke × every franchise across all time.** Build the Meta integration as a feature of the **PAR Spoke Skeleton** (the shared infrastructure layer that every PAR-affiliated app inherits), NOT as a per-spoke feature. The skeleton itself owns the Meta App; spokes plug into it. This is one level of abstraction beyond "one Meta App per spoke product" — we want one Meta App for the entire skeleton, so future spokes (and existing ones like Margin) inherit the integration without their own App Review.

The math of why this matters: spokes × franchises = many. "One App per spoke" still multiplies App Review load by spoke count. "One App for the skeleton" is a constant — review once, the next 50 spokes and the 5,000th franchise both benefit at zero marginal review cost. This is exactly how Meta's "Tech Provider" model is designed (HubSpot, Hootsuite, Salesforce all operate one Meta App across thousands of customer Pages — a tech provider IS a skeleton).

Five concrete pieces this changes from a naïve per-spoke design:

1. **The Meta App lives at PAR**, named something like "PAR Spoke Platform" or "Selectively Spokes" — not "PAR DK." It's owned by the PAR org. PAR holds the App ID + App Secret + the one configured webhook callback URL. App Review describes the platform's general use case ("we operate spoke applications that help educators / event-organizers / wellness-providers manage their Meta-sourced leads and customer conversations") with the screencast demonstrating one spoke as a representative example.

2. **PAR hosts a dispatcher Edge Function (`spoke-meta-webhook-router`)** that receives every Meta webhook (every spoke, every franchise, every Page) and forwards each event to the correct spoke's backend. The router:
   - Verifies the `X-Hub-Signature-256` HMAC against the PAR-held App Secret.
   - Inspects `entry[].id` (the Page ID) and looks up which spoke + which franchise org owns that Page via a PAR-side table `spoke_meta_pages (page_id, spoke_slug, franchise_org_id, ig_user_id, page_access_token_encrypted, connected_at)`.
   - Forwards the event payload to the resolved spoke's existing webhook endpoint (e.g., DK's `dk-meta-lead-webhook` for `leadgen`, future `dk-messenger-webhook` for `messages`) signed with `SPOKE_INSTALL_SIGNING_SECRET` (the same shared secret already used for the install handoff, §4.7) so the spoke can verify the forward came from PAR.
   - Each spoke retains its own business logic for processing the event — only the receive-and-verify boundary changes. DK's leads inbox UI, the promote-to-student RPC, the templated composer — all unchanged.

3. **The OAuth flow lives in PAR's Connected Apps panel.** When a franchise clicks **Connect Facebook** for a given spoke, PAR:
   - Hosts the OAuth callback (Meta only allows callback URLs on domains pre-registered with the App; PAR's domain is the natural canonical one).
   - Persists the resulting page token + metadata in `spoke_meta_pages`.
   - Notifies the target spoke's backend via the same install-handoff signing pattern that the spoke is now connected, so the spoke renders its Messenger / Leads inbox surfaces. Spokes never see Meta's App Secret — they see PAR's forward signature.

4. **Spoke-side schema converges across spokes.** The skeleton ships canonical schema templates (`conversations` / `conversation_messages` / `messenger_outbox` / `leads`) that each spoke's migrations bootstrap from. Each spoke's migrations can extend the canonical shape (DK might add `linked_student_id`; Margin might add `linked_invoice_id`) but the core columns stay the same so cross-spoke tooling — composers, inbox UIs, infographic-attach, the placeholder chip preview — can be lifted between spokes by name.

5. **`SPOKE_INSTALL_SIGNING_SECRET` becomes the universal trust boundary.** Today it covers the install handoff (§4.7). Post-skeleton it ALSO covers the PAR→spoke webhook forward, the OAuth-completion notification, and (likely) any future PAR→spoke telemetry channel. The spoke just needs to verify "this forward came from PAR." No spoke ever needs Meta's App Secret directly — that simplifies spoke-side env-var management and reduces the blast radius of any spoke compromise.

**Retroactive migration plan for existing spokes:**

- **PAR DK — T15 already shipped with the per-spoke env-var pattern.** Today `dk-meta-lead-webhook` receives directly from Meta with its own `META_APP_SECRET` env var on the DK Supabase project. The migration: PAR creates the skeleton-level Meta App, sets up the dispatcher, and Charleston re-connects via PAR's new Connected Apps OAuth flow. DK's webhook function gets a small patch — verify the PAR forward signature instead of Meta's signature, and read the page token via PAR-side lookup (or a cached `dk_config.meta_page_*` mirror) instead of `Deno.env.get()`. The leads inbox + reply UI + promote RPC don't change at all. Old env-var subscription is decommissioned. Charleston has to OAuth once (~30 seconds) — a fair tradeoff for inheriting App Review for everything else.
- **Margin** (the other shipped PAR spoke): if Margin already has any Meta integration, it migrates the same way. If Margin doesn't yet have one, it adopts the skeleton's pattern straight away when it next needs Meta touchpoints — no separate App Review, just a spoke-side `margin-*-webhook` that verifies the PAR forward signature.
- **Future spokes** (whatever PAR ships next — wellness, calendar, scheduler, anything): they inherit the Meta integration as a free out-of-the-box capability of the skeleton. Spec lookup, copy the canonical schema shape, write whatever spoke-specific business logic, point at the PAR dispatcher's forward URL. No App Review delay, no per-spoke Meta App registration.

**What "PAR Spoke Skeleton" includes beyond the Meta integration:**

This document is DK's CLAUDE.md so the skeleton description here is necessarily DK-flavored — the authoritative skeleton spec belongs in PAR's CLAUDE.md or a dedicated `SPOKE_SKELETON.md` (TODO: create that doc). The skeleton is the shared substrate that, in addition to the Meta integration described above, includes:
- **Identity federation** (already shipped) — `spoke-get-identity`, `spoke-create-org-invitation`, the HMAC-signed install handoff (§4.7), the auto-promotion-from-PAR-org-ownership pattern (§4.8). DK and Margin both consume these PAR Edge Functions today.
- **Audit-event federation** (already shipped — see DK's T6e) — outbox pattern for spoke→PAR audit-event delivery via `process_role_audit_outbox()` and `spoke-emit-audit`.
- **Canonical schema templates** for the cross-spoke surfaces above.
- **Shared UI component pattern library** — composer with placeholder chips + live preview + side-by-side editor (T15), tag-filterable infographic strip (T15), template-replied-and-marked-contacted flow. Today these live in DK's monolithic `app.js`; the skeleton's job is to extract them into a per-spoke-importable JS module so Margin (and future spokes) build with the same UX out of the box.

**Sharon-side checklist post-skeleton (replaces the per-franchise paste-into-env workflow):**
1. **One-time, PAR-side:** create the skeleton Meta App, set up the dispatcher Edge Function + `spoke_meta_pages` table, submit for App Review covering `leadgen` + `pages_messaging` + IG permission triad. App-Review screencast demonstrates DK as the example spoke.
2. **One-time, per-spoke:** spoke's webhook function adds PAR-forward signature verification (a few lines using the existing `SPOKE_INSTALL_SIGNING_SECRET`).
3. **Per-franchise, Sharon's side:** in PAR's Connected Apps panel, click **Connect Facebook** for PAR DK → Facebook OAuth → done. ~30 seconds, no Graph Explorer, no env vars.
...
1. **T16a — PAR Spoke Skeleton scaffolding + receive-only inbox.**
   - **PAR-side:** create the skeleton Meta App ("PAR Spoke Platform" or similar), build the `spoke-meta-webhook-router` dispatcher Edge Function, build the `spoke_meta_pages` table + the OAuth-flow page in PAR's Connected Apps panel.
   - **DK-side:** schema migration for `conversations` + `conversation_messages` + `messenger_outbox`, the `dk-messenger-webhook` function (verifies PAR forward signature, ingests messages), inbox tab UI showing incoming messages read-only (admin replies via the Meta-native UI for now). T15.5 piggy-backs: `dk-meta-lead-webhook` updated to verify the PAR forward signature instead of Meta's HMAC.
   - **No `pages_messaging` write scope needed yet** — the read scope is `pages_read_engagement`, much faster App Review (often days, not weeks).
   - **Net effect:** Charleston re-OAuths once via PAR's panel; the leads pipeline keeps working, and a new "Messages" tab shows incoming Messenger threads. App Review for the read scope is queued in parallel.
2. **T16b — Send path + composer.** Adds `dk-messenger-send`, the templated composer (mirroring T15's reply modal — placeholder chips, live preview, infographic attach), and the read-receipt mark-seen path. Requires `pages_messaging` App Review on the skeleton Meta App; once approved, every franchise across every spoke that uses the dispatcher inherits the approval.
3. **T16c — Instagram parity.** Mirrors T16a/b for IG. Same canonical schema (the `channel` column already accommodates), separate App Review for `instagram_manage_messages` on the same skeleton App. Independent — can sequence after T16b lands or in parallel with the App Review wait.
...
- Spoke registration in `public.spokes` (slug `dk`, name, install_url, etc.)
- API key in `public.spoke_api_keys` (SHA-256 hash stored, raw key only displayed at mint)
- Install row in `public.spoke_installations` for the franchise org
- **Capability row in `public.spoke_capabilities`** for the (`dk`, `spoke-create-task`) pair — this is a separate gate beyond install. **Margin's setup doc didn't flag this**; T17 turn-on rediscovered it. Without the capability row, `spoke-create-task` returns `403 capability_missing` even with a valid key + install. Add a parallel row when granting future endpoints (e.g. `spoke-emit-audit` if it ever moves behind capability gating)
- DK is single-tenant per deployment (one `dk_config` singleton row → one PAR org per Supabase project per Vercel project), structurally identical to Margin's "one operator → one PAR org" model. Multi-franchise auto-install is not part of T17.

**Two turn-on gotchas (T17b lessons learned):**

1. **`admin_install_spoke()` rejects SQL Editor calls.** The function reads `auth.uid()` to record `installed_by`, and Supabase SQL Editor runs as service-role where `auth.uid()` is null. The function raises `auth_required`. Either invoke it from PAR's signed-in admin UI, OR skip the RPC and directly INSERT into `spoke_installations` (`spoke_slug`, `org_id`, `installed_by` = your PAR user uuid). Margin's setup doc says "in the PAR admin UI" but doesn't flag the SQL Editor failure mode.
2. **Supabase SQL Editor doesn't preserve temp tables across statements.** PgBouncer in transaction mode bounces connections between statements, so `CREATE TEMP TABLE … ; INSERT … ; SELECT FROM …` fails with "relation does not exist." For multi-statement bootstraps that need to share a generated value (e.g. mint a raw key, store its hash, return the raw key), wrap in a `CREATE OR REPLACE FUNCTION` and call it via SELECT — functions are persistent across connections. The pattern lives in `T17_VERIFICATION.md` if needed for similar bootstraps.

Cross-franchise concerns (curriculum push from DK Corporate, etc.) live at a different federation layer and are explicitly out of scope here.
...
- **T17b (turn on)** — INSERT rows on PAR for `(spokes, spoke_api_keys, spoke_installations, spoke_capabilities)` covering slug `dk` and endpoint `spoke-create-task`, set `DK_SPOKE_API_KEY` in DK Edge Function secrets. Federation lights up. Direct INSERTs are the path because `admin_install_spoke()` rejects SQL Editor calls — see the gotchas above.
...
**T17a-4 — decisions + weight_ledger_entries (✅ shipped).** The engagement-doc YAML format had three structural sections beyond `workstreams[].tasks[]`: `decisions`, `weight_ledger`, `patterns`. T17a only landed tasks. T17a-4 lifts decisions and weight_ledger from "silently ignored" to first-class:

- Migration `phase_t17a4_engagement_extensions.sql` adds `public.decisions` (`title`, `description`, `not_before`, `inputs text[]`, `status`, `decided_at`, `decided_by`, `decided_outcome`, `external_ref`, project_name, audit fields) and `public.weight_ledger_entries` (`dated_at`, `item`, `moved_to`, `notes`, `source_task_id` FK to tasks on delete set null, `external_ref`, project_name). Both gated on existing `manage_tasks` permission, SELECT open to authenticated, both in `supabase_realtime`.
- YAML importer extended to upsert `decisions[]` and `weight_ledger[]` alongside tasks, idempotent on `(project_name, external_ref)`. Patterns stay informal (no DB rows) — they live in prose / repo docs.
- Tasks tab renders decisions as a violet-accent card group above tasks within each project. Per-card status select (pending → decided | cancelled), inline outcome textbox saved on blur. Reopening (decided → pending) clears the decided stamp.
